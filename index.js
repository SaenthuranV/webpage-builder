...
//OLD CODE ABOVE WITHOUT DB AUTHENTICATION
...
const corsConfig = {
  origin: "http://localhost:8081", // Updated to match your frontend origin
  credentials: true,
};
...
app.post("/login", function(request, response) {
  ...
  if (email === hardcodedEmail && password === hardcodedPassword) {
    request.session.start = new Date().getTime();
    request.session.newData = {
      _id: uuidv4(), // Use a unique ID for session
      Auth: true,
    };
    console.log(request.session.newData);
    response.json(request.session.newData);
    ...
    response.json({
      error: "Account information is incorrect or does not exist",
    });
  }
});

// Dummy endpoints for signup and other routes
app.post("/signup", function(request, response) {
  response.json({ message: "Signup endpoint is disabled in this mode." });
});

app.post("/traffic", function(request, response) {
  response.json({ message: "Traffic endpoint is disabled in this mode." });
});

app.post("/getData", function(request, response) {
  response.json({ message: "GetData endpoint is disabled in this mode." });
});
...
app.get("/verifyIfAdmin", function(request, response) {
  response.json({ Admin: request.session.Admin || false });
});
